--  ~/.local/share/nvim/lazy/ui/lua/nvchad/colorify/methods.lua
...
M.lsp_var = function(buf, line, min, max)
  ...
  buf = type(buf) == "function" and buf() or (buf or vim.api.nvim_get_current_buf())
  for _, client in pairs(vim.lsp.get_clients { bufnr = buf }) do
    if client.server_capabilities.colorProvider then
        -- Asegurar que bufnr sea un número válido
	local safe_bufnr = type(bufnr) == "number" and bufnr or vim.api.nvim_get_current_buf()
	vim.lsp.buf_request(safe_bufnr, "textDocument/documentColor", params, function(err, result)if resp and line then
